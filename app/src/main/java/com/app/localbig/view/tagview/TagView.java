package com.app.localbig.view.tagview;
...
import com.app.localbig.view.tagview.drawers.ClassicTagDrawer;
import com.app.localbig.view.tagview.drawers.ModernSharpTagDrawer;
import com.app.localbig.view.tagview.drawers.ModernTagDrawer;
import com.app.localbig.view.tagview.drawers.ReversedModernSharpTagDrawer;
import com.app.localbig.view.tagview.drawers.ReversedModernTagDrawer;
import com.app.localbig.view.tagview.drawers.ReversedSharpTagDrawer;
import com.app.localbig.view.tagview.drawers.SharpTagDrawer;
...
public class TagView extends TextView {
    ...
    private class TagDrawable extends Drawable {
        ...
        public void draw(Canvas canvas) {
            ...
            } else if (data.tagType == Tags.MODERN) {
                setPadding(data.tagLeftPadding * Tags.MODERN_TAG_MULTIPLIER, data.tagTopPadding,
                        data.tagRightPadding, data.tagBottomPadding);
                new ModernTagDrawer().drawTag(bounds, canvas, data);
            } else if (data.tagType == Tags.REVERSED_MODERN) {
                setPadding(data.tagLeftPadding, data.tagTopPadding,
                        data.tagRightPadding * Tags.MODERN_TAG_MULTIPLIER, data.tagBottomPadding);
                new ReversedModernTagDrawer().drawTag(bounds, canvas, data);
            } else if (data.tagType == Tags.SHARP) {
                setPadding(data.tagLeftPadding, data.tagTopPadding,
                        data.tagRightPadding * Tags.SHARP_TAG_MULTIPLIER, data.tagBottomPadding);
                new SharpTagDrawer().drawTag(bounds, canvas, data);
            } else if (data.tagType == Tags.REVERSED_SHARP) {
                setPadding(data.tagLeftPadding * Tags.SHARP_TAG_MULTIPLIER, data.tagTopPadding,
                        data.tagRightPadding, data.tagBottomPadding);
                new ReversedSharpTagDrawer().drawTag(bounds, canvas, data);
            } else if (data.tagType == Tags.MODERN_SHARP) {
                setPadding(data.tagLeftPadding * Tags.MODERN_TAG_MULTIPLIER, data.tagTopPadding,
                        data.tagRightPadding * Tags.SHARP_TAG_MULTIPLIER, data.tagBottomPadding);
                new ModernSharpTagDrawer().drawTag(bounds, canvas, data);
            } else if (data.tagType == Tags.REVERSED_MODERN_SHARP) {
                setPadding(data.tagLeftPadding * Tags.SHARP_TAG_MULTIPLIER, data.tagTopPadding,
                        data.tagRightPadding * Tags.MODERN_TAG_MULTIPLIER, data.tagBottomPadding);
    ...
    public TagView(Context context, AttributeSet attrs) {
        ...
        try {
            data.tagType = typedArray.getInteger(R.styleable.TagView_tagType, Tags.CLASSIC);
            ...
            data.tagBorderRadius = typedArray.getInteger(R.styleable.TagView_tagBorderRadius, Tags.BORDER_RADIUS_DEFAULT);
            data.tagCircleRadius = typedArray.getInteger(R.styleable.TagView_tagCircleRadius, Tags.CIRCLE_RADIUS_DEFAULT);
            data.tagCircleColor = typedArray.getColor(R.styleable.TagView_tagCircleColor, Tags.CIRCLE_COLOR_DEFAULT);
            data.tagTextColor = typedArray.getColor(R.styleable.TagView_tagTextColor, Tags.TEXT_COLOR_DEFAULT);
        } finally {
            typedArray.recycle();
        }
    ...
    private void initPadding() {
        ...
        if(left == 0) data.tagLeftPadding = Tags.LEFT_PADDING_DEFAULT;
        else data.tagLeftPadding = left;
        if(right == 0) data.tagRightPadding = Tags.RIGHT_PADDING_DEFAULT;
        else data.tagRightPadding = right;
        if(top == 0) data.tagTopPadding = Tags.TOP_PADDING_DEFAULT;
        else data.tagTopPadding = top;
        if(bottom == 0) data.tagBottomPadding = Tags.BOTTOM_PADDING_DEFAULT;
